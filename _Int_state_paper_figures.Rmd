...
TORC2<-read.csv(file.path(pathname, "extdata/1B_rict-1_TORC2.csv"), header=TRUE) %>% format_dauer(p.dauer = "non") #including partial/pd as non-dauer due to excess zeros. 
...
d7GFP<-read.csv("extdata/2A_daf7GFP.csv") %>% filter(mean!=4095 & genotype %in% strains & date %in% dates & temp == "27" & food == "OP50") %>% mutate(genotype = factor(genotype, levels=strains), ID = as.character(ID)) %>%
...
d7GFP<-read.csv(file.path(pathname, "extdata/2A_3A_daf7GFP.csv")) %>% filter(mean!=4095 & genotype %in% strains & date %in% days & pheromone == 0 & food == "OP50") %>% mutate(genotype = factor(genotype, levels=strains), ID = as.character(ID)) %>%
  separate(ID, c("ID.A", "ID.B"), sep = ":", extra = "drop") %>% 
...
df <- d28 %>% subset(day %in% tempdays & food %in% foods & mean !=4095 & genotype == "N2") %>% group_by(date,temp,genotype,neuron,genoID) %>% summarise(cell.norm = mean(mean)) %>%
  data.frame() %>% mutate(group.id = interaction(genotype, neuron)) # take mean of each worm
...
p2 <- df %>% ggplot(aes(x=exp(temp), y=log10(cell.norm))) + geom_boxplot(aes(group = temp)) + geom_point(alpha = 0.2)
...
anova(lm.ASI.lin,lm.ASI.poly.2)
lm.ASI.log <- with(log.tran, lm(linkfun(mean) ~ log10(temp), data = subset(d28, day %in% tempdays & food == "OP50" & mean !=4095 & genotype == "N2" & neuron == "ASI")))
anova(lm.ASI.lin)
...
newdata = data.frame(temp = seq(20,27, by = 0.1), genoID = rep(0,71))

predict(lm.ASI.poly.2, newdata = newdata, re.form = NA)
...
filepath<-file.path(pathname, "extdata/6A_wildHTdata_allbinary.csv")
foods = "OP50"
...
filepath<-file.path(pathname, "extdata/S3B_C3_sumstats.csv")
...
wild.roam<-read.csv(file.path(pathname, "extdata/6B_7F_roam_wild_isolates.csv")) %>%
