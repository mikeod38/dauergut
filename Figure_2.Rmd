...
(p<-(df %>% ggplot(aes(x=genotype, y = cell.norm)) +
       ...
       geom_errorbar(data=mixed, aes(x=x.pos,y=mean, ymin=lower.CL, ymax=upper.CL),
                  width=0,colour ="grey", lwd=0.2) +
       geom_errorbar(data=mixed, aes(x=x.pos,y=mean, ymin = lower.25, ymax = upper.75),
                  width=0,colour = "darkgrey", lwd = 0.9) +
       geom_segment(data = mixed, aes(x = x.pos-(0.007*nrow(mixed)), y = mean, xend = x.pos+(0.007*nrow(mixed)),
...
cbind(ASI,ASJ) %>% `colnames<-`(c('ASI','ASJ')) %>% knitr::kable()
...
#get odds-ratio
knitr::kable(mfx::logitor(glm.simple, data = rictC3)$oddsratio) # OR ~ 21.3
```
