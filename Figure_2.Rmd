...
# ### Linear models ###
# lsm_geno_tukey <- purrr::compose(
#   prange,
#   summary,
#   purrr::partial(lsmeans::contrast, method = "pairwise"),
#   purrr::partial(lsmeans::lsmeans, specs = "genotype"),
#   purrr::partial(lsmeans::ref.grid, transform = "response", data = df),
#   lm)
       
                            

#lmm2<-with(log.tran, lmer(linkfun(cell.norm) ~ genotype + (1|date) + (1|genotype:date), data = df))
# df_1 <- df %>% filter(dataset == "1") %>% filter(outlier.status == FALSE) %>% droplevels()
# df_2 <- df %>% filter(dataset == "2") %>% filter(outlier.status == FALSE) %>% droplevels()
# 
# linmod1 <- with(log.tran, lm(linkfun(cell.norm) ~ genotype, data = df_1))
linmod2 <- with(log.tran, lm(linkfun(cell.norm) ~ genotype, data = df_2))
linmod <- with(log.tran, lm(linkfun(cell.norm) ~ genotype, data = df[df$outlier.status == FALSE,]))
# stanlmer_1 <- with(log.tran, rstanarm::stan_lmer(linkfun(cell.norm) ~ genotype + (1|date) + (1:genotype:date), 
#                                                data = df_1, 
#                                                chains = 3, 
#                                                cores =4, 
#                                                seed = 2000,
...
# stanlmer_2 <- with(log.tran, rstanarm::stan_lmer(linkfun(cell.norm) ~ genotype + (1|date) + (1:genotype:date), 
#                                                data = df_2, 
#                                                chains = 3, 
#                                                cores =4, 
#                                                seed = 2000,
...
stanlmer <- with(log.tran, rstanarm::stan_lmer(linkfun(cell.norm) ~ genotype + (1|date) + (1:genotype:date), 
                                               ...
                                               iter=6000,
                                               control = list(adapt_delta=0.99)))
### compare ft7 ges-1 to rescue H0, ft7 = ft7_resc ###


contrasts <- tukey_contrasts(linmod, factor = "genotype")
linmod %>% lsmeans::ref.grid(., type = "response") %>% lsmeans::lsmeans(.,"genotype") %>% lsmeans::contrast(method = "pairwise", adjust = "mvt")
...
#contrasts.1 <- linmod1 %>% tukey_contrasts(factor = "genotype")
#contrasts.2 <- linmod2 %>% tukey_contrasts(factor = "genotype")

# plot.contrasts_N2_d1<-c("",contrasts.1$prange[1:3])
# plot.contrasts_ft7_d1 <- c("", "", "", contrasts.1$prange[6])
# 
# plot.contrasts_N2_d2<-c("",contrasts.2$prange[1:3])
# plot.contrasts_ft7_d2 <- c("", "", contrasts.2$prange[4:5])

# strains <- c("N2","mg360","ft7","ft7_ges1resc")
# estimates_1 <- stanlmer_1 %>% getStan_CIs(type = "log", base = 10)
# strains <- c("N2","ft7","ft7_ifb2resc", "ft7_gpa4resc")
# estimates_2 <- stanlmer_2 %>% getStan_CIs(type = "log", base = 10)

#df %>% ggplot(aes(x = genotype, y = cell.norm)) + geom_point(aes(colour = date)) + add.median() + add.quartiles()
...
labels_1 <- labels[1:4]
labels_2 <- labels[c(1,3,5:6)]

mixed <- estimates_1
p1 <- df_1 %>% filter(dataset == "1") %>%
  dauergut::plot_CIs(plot.contrasts = plot.contrasts_N2_d1, plot.contrasts.2 = plot.contrasts_ft7_d1, type= "GFP", title = "", labels = labels_1, ypos = 6500, offset = 500)

mixed <- estimates_2
p2 <- df_2 %>% filter(dataset == "2") %>%
  dauergut::plot_CIs(plot.contrasts = plot.contrasts_N2_d2, plot.contrasts.2 = plot.contrasts_ft7_d2, type= "GFP", title = "", labels = labels_2, ypos = 6500, offset = 500) + 
  theme(
...
gridExtra::grid.arrange(p1,p2, nrow = 1, top = "Intestinal TORC2 regulates daf-7::GFP expression", widths = c(4,3.7))
```
...
```{r 2A daf-7 stats}
library(sjPlot)
sjt.lm(linmod1, depvar.labels = "log(mean GFP intensity) (AU)", show.se = TRUE, show.fstat = TRUE)
sjt.lm(linmod2, depvar.labels = "log(mean GFP intensity) (AU)", show.se = TRUE, show.fstat = TRUE)
knitr::kable(contrasts.1, caption="Pairwise comparisons from ANOVA (Dunnett)")
knitr::kable(contrasts.2, caption="Pairwise comparisons from ANOVA (Dunnett)")
knitr::kable(estimates_1[,c(6,1:5)], caption = "Bayesian credible intervals")
knitr::kable(estimates_2[,c(6,1:5)], caption = "Bayesian credible intervals")
```
